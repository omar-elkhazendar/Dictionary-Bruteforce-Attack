...
def load_password_dictionary(file_path):
    ...
    for encoding in encodings:
        try:
            with open("./list.txt", "r", encoding=encoding) as file:
...
def open_welcome_page():
    ...
    welcome_window.configure(bg="#F5F5F5")  # Light background

    welcome_label = ttk.Label(welcome_window, text="Welcome!", font=("Helvetica", 20, "bold"), background="#F5F5F5", foreground="#333333")
...
style.configure("TFrame", background="#F5F5F5")  # Light background for frames
style.configure("TLabel", background="#F5F5F5", foreground="#333333", font=("Helvetica", 12))  # Dark text on light background
style.configure("TButton", background="#E0E0E0", foreground="#333333", font=("Helvetica", 12), padding=10)
style.map("TButton", background=[("active", "#BDBDBD")])  # Change button color when active (clicked)

# Custom style for the login button
style.configure("Accent.TButton", background="#64B5F6", foreground="#FFFFFF", font=("Helvetica", 12, "bold"))
